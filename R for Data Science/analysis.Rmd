...
Notice that we can apply the logarithm to the energy usage data because they
are all positive. We get an empty tibble when filtering for energy values less
...
the data are independent due to the space and time separation.
...
as having a smaller variance than the others. Despite this, we will continue.
...
is, we reject that the log of the group means are equal. Since the logarithm is
a monotone increasing function, we also reject that the group means are equal.
